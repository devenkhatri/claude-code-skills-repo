...
Transform any codebase into a stunning, interactive single-page HTML course. The output is a single self-contained HTML file (no dependencies except Google Fonts) that teaches how the code works through scroll-based modules, animated visualizations, and plain-English translations of code.
...
>
> I'll read through the code, figure out how everything fits together, and generate a beautiful single-page HTML course with animated diagrams and plain-English code explanations. The whole thing runs in your browser — no setup needed.
...
- At least one interactive element (visualization, or animation)
...
A module with only text and code blocks, no interactivity. Every module needs at least one of: data flow animation, group chat, architecture diagram, drag-and-drop. These aren't decorations — they're how non-technical learners actually process information.
